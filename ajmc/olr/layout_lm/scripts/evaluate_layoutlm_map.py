...
from transformers import LayoutLMv3TokenizerFast, LayoutLMv3ForTokenClassification, LayoutLMv3FeatureExtractor, RobertaTokenizerFast, RobertaForTokenClassification
    ...
    for page in pages:
        ...
        for i in range(len(words)):
            if i == 0:
                ...
                if labels[i] != labels[i - 1] or words[i].bbox.xyxy[-1] < (words[i-1].bbox.xyxy[1]-50): # for double col
        ...
        for region in pred_regions:
            if any([is_rectangle_within_rectangle(region['bbox'].bbox, r['bbox'].bbox)
        ...
        for region in pred_regions:
            draw.rounded_rectangle(xy=region['bbox'].bbox_2,
                                   outline=labels_to_colors[region['label']], radius=4,
                                   width = 10)

        img = draw_caption(img, labels_to_colors=labels_to_colors, font_dir='/mnt/ajmcdata1/drive_cached/AjaxMultiCommentary/dissemination/media/fonts')
