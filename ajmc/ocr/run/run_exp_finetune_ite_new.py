...
for ite in [1500, 2000]:
    print(f"Current ite: {ite}")
    finetune_suffix = "clean-threshold-1.0-resize-30"
    train(
        f"finetune-grc-pogretra-{finetune_suffix}-ite{ite}-valid0.9-lr0.0001-new", 
        [f"{finetune_suffix}-merge"], 
        mode, 
        TESSDATA_BEST_DIR, 
        config_file=os.path.join(PROJECT_DIR, "ocr", "tesstrain_configs", f"finetune_ite{ite}_valid0.9_lr0.0001.yaml"), 
        cleaned_suffix="")

    custom_ckpt = f"finetune-grc-pogretra-{finetune_suffix}-ite{ite}-valid0.9-lr0.0001-new"
